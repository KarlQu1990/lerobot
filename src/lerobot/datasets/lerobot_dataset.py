    ...
    def update_video_info(self, video_key: str | None = None) -> None:
        ...
        for key in video_keys:
            if not self.features[key].get("info", None):
                video_path = self.root / self.video_path.format(video_key=video_key, chunk_index=0, file_index=0)
...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def _check_cached_episodes_sufficient(self) -> bool:
        ...
        available_episodes = {
            ep_idx.item() if isinstance(ep_idx, torch.Tensor) else ep_idx for ep_idx in self.hf_dataset["episode_index"]
        }
