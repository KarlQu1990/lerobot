...
class OpenCVCameraConfig(CameraConfig):
    ...
    def __post_init__(self):
        ...
        if platform.system() == "Windows":
            from lerobot.utils.usb_utils import USBDeviceManager, get_camera_device_paths

            if not isinstance(self.index_or_path, int):
                device_paths = get_camera_device_paths()
                mgr = USBDeviceManager().load()

                index_or_path = str(self.index_or_path)
                instance_id: str = mgr.devices[index_or_path]["Instance ID"]
                # 反斜杠替换成#
